...
class MADDPG_QMIX_Learner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        original_rewards = batch["reward"][:, :-1]
        actions = batch["actions_onehot"]
        original_terminated = batch["terminated"][:, :-1].float()
        rewards = original_rewards.unsqueeze(2).expand(-1, -1, self.n_agents, -1)
        terminated = original_terminated.unsqueeze(2).expand(-1, -1, self.n_agents, -1)
        
        #mask = 1 - terminated

        mask = batch["filled"][:, :-1].float()
        mask[:, 1:] = mask[:, 1:] * (1 - original_terminated[:, :-1])
        ...
        targets = original_rewards.reshape(-1, 1) + self.args.gamma * (1 - original_terminated.reshape(-1, 1)) * mixed_target_vals.reshape(-1, 1).detach()
        if self.args.standardise_returns:
