...
class MADDPG_VDN_Learner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        targets = rewards.reshape(-1, 1) + self.args.gamma * (1 - terminated.reshape(-1, 1)) * mixed_target_vals.reshape(-1, 1).detach()
        ...
        masked_td_error = td_error * mask.reshape(-1, 1) #FOR PRED PREY
        ...
        # Compute the actor loss with respect to the q and policy pi
        #pg_loss = -(q * mask.reshape(1, -1)).mean() + self.args.reg * (pis ** 2).mean()
        pg_loss = -(q * mask).mean() + self.args.reg * (pis ** 2).mean() #THIS IS FOR PRED PREY
        # Optimise agents
